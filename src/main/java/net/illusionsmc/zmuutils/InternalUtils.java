...
public class InternalUtils {
    ...
    static INamedMappingFile loadNamed(InputStream in) throws IOException {
        ...
        if ("PK:".equals(test) || "CL:".equals(test) || "FD:".equals(test) || "MD:".equals(test)) //SRG
            return loadZmu(filter(lines)).build();
        ...
        else if (firstLine.startsWith("tzmu2 ")) // TSRG v2, parameters, and multi-names
            return loadTZmu2(filter(lines)).build();
        else // TSRG/CSRG
            return loadSlimZmu(filter(lines)).build();
    }

    private static IMappingBuilder loadZmu(List<String> lines) throws IOException {
    ...
    private static IMappingBuilder loadProguard(List<String> lines) throws IOException {
        ...
        for (String line : lines) {
            ...
            if (!line.startsWith("    ") && line.endsWith(":")) {
                ...
                if (cls == null) throw new IOException("Invalid PG line, missing class: " + line);
    ...
    private static IMappingBuilder loadSlimZmu(List<String> lines) throws IOException {
        IMappingBuilder ret = IMappingBuilder.create("left", "right");
        Map<String, IMappingBuilder.IClass> classes = new HashMap<>();
        ...
        for (String line : lines) {
            ...
            if (pts[0].charAt(0) == '\t') {
                if (cls == null)
                    throw new IOException("Invalid TZMU line, missing class: " + line);
                ...
                if (pts.length == 2)
                    ...
                    throw new IOException("Invalid TZMU line, to many parts: " + line);
            } else {
                if (pts.length == 2) {
                    if (!pts[0].endsWith("/"))
                    ...
                    throw new IOException("Invalid CZMU line, to many parts: " + line);
            }
        }
    ...
    private static IMappingBuilder loadTZmu2(List<String> lines) throws IOException {
        /*
        ...
        if (header.length < 3) throw new IOException("Invalid TZmu v2 Header: " + lines.get(0));
        ...
        for (String line : lines) {
            if (line.length() < 2)
                throw new IOException("Invalid TZmu v2 line, too short: " + line);

            String[] pts = line.split(" ");
            if (line.charAt(0) != '\t') { // Classes or Packages are not tabbed
                if (pts.length != nameCount)
                    throw new IOException("Invalid TZMU v2 line: " + line);
                ...
                if (mtd == null)
                    throw new IOException("Invalid TZMU v2 line, missing method: " + line);
                ...
                if (pts.length == 1 && pts[0].equals("static"))
                    ...
                    throw new IOException("Invalid TZMU v2 line, too many parts: " + line);
            } else {
                if (cls == null)
                    throw new IOException("Invalid TZMU v2 line, missing class: " + line);
                pts[0] = pts[0].substring(1);
                ...
                if (pts.length == nameCount) // Field without descriptor
                    ...
                    throw new IOException("Invalid TZMU v2 line, to many parts: " + line);
            }
        }
